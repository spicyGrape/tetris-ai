...
class Tetris:
    ...
    def _clear_lines(self, board):
        ...
        lines_to_clear = [
            index for index, row in enumerate(board) if sum(row) == Tetris.BOARD_WIDTH
        ]
        if lines_to_clear:
            board = [
                row for index, row in enumerate(board) if index not in lines_to_clear
            ]
    ...
    def play(self, x, rotation, render=False, render_delay=None):
        ...
        score = 0
        if self.used_tetrominos >= 400:
            self.game_over = True
            print(self.score)
        else:
            self.used_tetrominos += 1
    ...
    def render(self):
        ...
        img = (
            np.array(img)
            .reshape(Tetris.BOARD_HEIGHT, Tetris.BOARD_WIDTH, 3)
            .astype(np.uint8)
        )
        ...
        img = np.array(img)
        cv2.putText(
            img, str(self.score), (22, 22), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1
        )
        cv2.putText(
            img,
            str(self.used_tetrominos),
            (44, 44),
